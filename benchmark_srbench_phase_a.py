"""
Phase A: Kamienny E2E vs DeSTrOI+Kamienny on 10 SRBench ground-truth problems.

Per problem runs both methods (same train/test split):
  1. Transformer alone
  2. DeSTrOI operator mask → Transformer

DeSTrOI uses a 2D slice of (X, y): for D>2, the two features with highest |corr(y)|.

Run:
    python3 benchmark_srbench_phase_a.py --download
    python3 benchmark_srbench_phase_a.py --n 1              # smoke test
    python3 benchmark_srbench_phase_a.py                    # full 10 (~2 h CPU)

Output (per subset file):
    results/srbench/phase_a_<subset_name>.csv
    results/srbench/phase_a_<subset_name>_summary.txt

Examples:
    python3 benchmark_srbench_phase_a.py --subset datasets/srbench/benchmark_subset_10.json
    python3 benchmark_srbench_phase_a.py --subset datasets/srbench/benchmark_subset_10_b.json
"""
...
SUBSET_PATH = os.path.join(ROOT, "datasets", "srbench", "benchmark_subset_10.json")


def paths_for_subset(subset_path: str) -> tuple[str, str]:
    """CSV + summary paths derived from subset filename."""
    tag = os.path.splitext(os.path.basename(subset_path))[0]
    csv_path = os.path.join(SRBENCH, f"phase_a_{tag}.csv")
    sum_path = os.path.join(SRBENCH, f"phase_a_{tag}_summary.txt")
...
def pick_destroi_slice(X: np.ndarray, y: np.ndarray, seed: int, n_max: int = N_DESTROI_POINTS):
    """Return (x1, x2, y) arrays for DeSTrOI (2D image encoding)."""
...
def summarize(rows: list[dict]) -> str:
    ...
    lines = [
        "Phase A — Kamienny E2E vs DeSTrOI+Kamienny (SRBench ground-truth)",
        f"Problems: {len({r['problem'] for r in rows})}",
        "",
    ]
    ...
    for method, mrows in [("e2e", by_method.get("e2e", [])), ("destroi_e2e", by_method.get("destroi_e2e", []))]:
        ...
        lines += [
            ...
            f"  Mean fit time: {np.nanmean([r['fit_sec'] for r in mrows]):.1f}s",
            "",
        ]

    if by_method.get("e2e") and by_method.get("destroi_e2e"):
        e2e = {r["problem"]: r["r2_test"] for r in by_method["e2e"]}
        comb = {r["problem"]: r["r2_test"] for r in by_method["destroi_e2e"]}
        deltas = []
        for p in e2e:
            if p in comb and np.isfinite(e2e[p]) and np.isfinite(comb[p]):
                deltas.append(comb[p] - e2e[p])
        if deltas:
            d = np.array(deltas)
            lines += [
    ...
    acc = [r["destroi_acc"] for r in by_method.get("destroi_e2e", []) if r.get("destroi_acc") is not None]
    if acc:
    ...
    for p in problems:
        e = next((r for r in rows if r["problem"] == p and r["method"] == "e2e"), None)
        c = next((r for r in rows if r["problem"] == p and r["method"] == "destroi_e2e"), None)
        if e and c:
            delta = c["r2_test"] - e["r2_test"] if np.isfinite(e["r2_test"]) and np.isfinite(c["r2_test"]) else float("nan")
            lines.append(
                f"  {p:<28s}  E2E={e['r2_test']:7.4f}  Comb={c['r2_test']:7.4f}  "
                f"Δ={delta:+.4f}  acc={c.get('destroi_acc', 0):.0%}  blocked={c.get('blocked', '')}"
            )

    # By benchmark group (compare to published success *rates*, not per-problem R²)
    ...
    for bench in ("feynman", "strogatz"):
        ...
        if not mrows:
            continue
        r2v = np.array([r["r2_test"] for r in mrows], dtype=float)
        valid = np.isfinite(r2v)
        r2v = r2v[valid]
    ...
    lines.extend([
        "",
        "NOTE: Published Kamienny numbers are success RATES over ALL problems",
        "  (Feynman 84.8% = 84.8% of 119 equations solved), NOT average R².",
        "  This run uses 10 hand-picked problems — not comparable to full SRBench.",
        "  Published Strogatz E2E: 35.7% (≈5/14). Run all 119+14 to reproduce.",
    ])
    return "\n".join(lines)
...
def run_problem(
    ...
    X, y, feature_names, info = read_problem(problem)
    print(
    ...
    x1, x2, y_dest = pick_destroi_slice(x_fit if y_fit.ndim == 1 else x_fit, y_fit.ravel(), seed)
    y_clip = np.clip(y_dest, *np.percentile(y_dest, [1, 99]))
    scores, _, absent = destroi.predict(x1, x2, y_clip, threshold=THRESHOLD)
    true_ops = gt_destroi_ops(info["ops"])
    ...
    out_rows = []
    for method, forb, label in (
        ("e2e", None, "E2E"),
        ("destroi_e2e", forbidden, "DeSTrOI+E2E"),
    ):
        print(
            f"    → [{i+1}/{n_total}] {problem} — {label} (fitting, may take several min)…",
            flush=True,
        )
        try:
            est = make_estimator(
        ...
        out_rows.append({
            **base,
            ...
            "r2_test": r2,
            "predicted": pred,
            "fit_sec": round(fit_sec, 2) if np.isfinite(fit_sec) else fit_sec,
            "error": err,
        })
        flag = "✓" if r2 >= R2_GOOD else "~" if r2 >= R2_OK else "✗"
        print(
            f"  [{i+1:2d}/{n_total}] {label:<12s} {flag} R²={r2:7.4f}  "
            f"{info['benchmark']:<8s} D={info['n_features']}  {problem}",
            flush=True,
        )
    return out_rows
...
def run(
    ...
    if csv_path is None:
        csv_path, _ = paths_for_subset(SUBSET_PATH)
    ...
    for i, problem in enumerate(problems[start:], start=start):
        new_rows = run_problem(
            problem, i, len(problems), torch_model, destroi,
            seed, n_trees, max_input_points, max_number_bags, max_train_rows,
        )
        ...
        print(f"    (~{eta/60:.1f} min left for {len(problems)-i-1} problems)\n", flush=True)

    return rows


def main():
    p = argparse.ArgumentParser(description="Phase A: E2E vs DeSTrOI+E2E on SRBench subset")
    p.add_argument("--download", action="store_true")
    p.add_argument("--subset", default=SUBSET_PATH, help="JSON list of problem names")
    p.add_argument("--output-csv", default=None, help="Override CSV output path")
    p.add_argument("--output-summary", default=None, help="Override summary output path")
    p.add_argument("--n", type=int, default=None, help="Limit to first N problems in subset")
    ...
    csv_path, sum_path = paths_for_subset(args.subset)
    if args.output_csv:
        csv_path = args.output_csv
    if args.output_summary:
        sum_path = args.output_summary

    if args.download:
        print(f"Downloading {len(problems)} datasets…")
        download_all(problems)
        if args.n is None and args.start == 0 and not args.subset:
            return

    print(f"\nPhase A: {len(problems)} problems (E2E + DeSTrOI+E2E each)")
